...
class SwinTransformerBlock4D(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size, shift_size, mlp_ratio = 4.0, qkv_bias = True, drop = 0.0, attn_drop = 0.0, drop_path = 0.0, act_layer = "GELU", norm_layer = nn.LayerNorm, use_checkpoint = False):
        """
        Args:
            dim: number of feature channels.
            num_heads: number of attention heads.
...
class PatchMergingV2(nn.Module):
    ...
    def __init__(self, dim, norm_layer = nn.LayerNorm, spatial_dims = 3, c_multiplier = 2):
        """
        Args:
            dim: number of feature channels.
            norm_layer: normalization layer.
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, window_size, drop_path, mlp_ratio = 4.0, qkv_bias = False, drop = 0.0, attn_drop = 0.0, norm_layer = nn.LayerNorm, c_multiplier = 2, downsample = None, use_checkpoint = False):
        """
        Args:
            dim: number of feature channels.
...
class SwinTransformer4D(nn.Module):
    ...
    def __init__(self, img_size, in_chans, embed_dim, window_size, first_window_size, patch_size, depths, num_heads, mlp_ratio = 4.0, qkv_bias = True, drop_rate = 0.0, attn_drop_rate = 0.0, drop_path_rate = 0.0, norm_layer = nn.LayerNorm, patch_norm = False, use_checkpoint = False, spatial_dims = 4, c_multiplier = 2, last_layer_full_MSA = False, downsample="mergingv2", num_classes=2, to_float = False, **kwargs):
        """
        Args:
